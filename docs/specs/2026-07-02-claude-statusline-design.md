...
**单文件 bash 脚本 `statusline.sh`**（`#!/bin/bash`）+ 系统自带 `jq`。

- 理由：Claude Code 的 statusline 是「运行一条命令、把 JSON 从 stdin 喂入、显示打印的一行」
  这一官方扩展点。bash + jq 是往这个钩子里塞的最轻量程序：**零构建、零工具链、零 pip/npm**。
- 依赖全部为 macOS 系统自带：`/bin/bash`（3.2）、`/usr/bin/jq`（macOS 15+ 内置）、
  `/usr/bin/git`。因此「重装即用」最彻底：`clone → ./install.sh` 即可，无需装任何东西。
- 兼容性：只用 bash 3.2 特性（大写用 `tr`、取整用 `$(( ))`），不用 bash4+ 语法。
- jq 负责解析全部字段，包括嵌套 `rate_limits.*` 与多态 `effort`
  （`.effort | if type=="string" then . else .level end`）。
- 前提/限制：依赖 macOS 自带 jq（15+）。若移植到旧版 macOS / 无 jq 的 Linux，需先装 jq；
  当前目标平台（本机 macOS）无需任何安装。
- 性能：每次渲染起若干短进程（bash + jq + git），远快于渲染节奏，肉眼无感；`git`
  子进程本就是主要耗时，与语言选择无关。
...
`effort` 字段为多态（字符串 **或** `{level}` 对象）：用 jq `if type=="string" then . else .level end` 兼容解析。
...
```
claude-statusline/
├── statusline.sh      # 脚本本体（bash + jq）
├── install.sh         # 合并 settings.json 的 statusLine；chmod +x
├── README.md          # 说明 + 重装步骤
├── preview.html       # 参考预览留档
└── docs/specs/        # 本设计文档
```
...
1. `chmod +x statusline.sh`；校验依赖 `jq` / `git` 存在（缺失则报错提示）。
2. 用 `jq` 安全合并 `~/.claude/settings.json`：设置
...
- 提供若干条固定 stdin JSON 样例（对应 preview.html 的 5 种状态：全安全 / context 逼近 /
  5H 偏高 / 7D 逼近 / 全红），`echo '<json>' | ./statusline.sh` 目测输出。
...
- 面向本机 macOS（自带 jq/git/bash）。旧版 macOS / Linux 若无 jq 需自行安装，暂不额外适配。
